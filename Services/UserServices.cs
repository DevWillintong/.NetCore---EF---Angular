...
namespace ImagineDreams.Services
{
    ...
    public class UserServices : IUserServices
    {
        ...
        public async Task<UserEntity?> getUserByEmail(string email)
        {
            try
            {
                 UserEntity? t =  await _userDatabaseContext.Users.Where(x => x.Email == email).FirstOrDefaultAsync();
                return t;
            }
            catch (Exception)
            {
        ...
        {
            UserEntity? userP = await _userDatabaseContext.Users.Where(x => x.Id == id).FirstOrDefaultAsync();
            if (userP == null)
            {
        ...
        public async Task<UserLoginResponse?> login(UserLoginRequest u)
        {
            ...
            var userC = await _userDatabaseContext.Users.Where(x => x.Email == u.Email.ToLower() && x.Password == encrypt(u.Password)).FirstOrDefaultAsync();
            if (userC == null)
            {
                return null;
            }
